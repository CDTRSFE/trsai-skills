...
3. 先自动运行扫描脚本，再读取最新的 `docs/component-map.md`，不要要求用户手动执行。文件不存在时初始化；已存在时默认合并更新并重新排序，保留补充的用途说明、备注和设计记录。
...
10. 开发完成后或提交前，必须重新运行扫描脚本，将新增/删除组件、接口字段、引用数量和位置、表格排序实际写回 `docs/component-map.md`；随后处理用途变化候选、公共化评估及设计记录。只输出差异报告不算完成维护。
...
1. 确认业务项目根目录及其 `src/` 存在。
2. 无论 `docs/component-map.md` 是否存在，自动运行：

```bash
node <skill-dir>/scripts/scan-vue-components.mjs <project-root>
```

3. 脚本在文件不存在时创建 `docs/` 并初始化索引；已存在时合并更新组件表格，实际写回变化。
...
7. 开发完成后或提交前再次运行同一命令，同步本次改动；如果只有顺序错误也必须修正。内容和排序均无变化时不重复写入。
...
Agent 使用本 skill 处理老工程、UI 开发或组件开发需求时，必须自动运行扫描脚本；不要让用户手动创建 `docs/` 或执行脚本。相同命令同时负责首次生成和后续更新：
...
```text
docs/component-map.md（不存在时生成；已存在时合并更新并重排；无变化时不重复写入）
```
...
- 自动维护：组件增删、名称、所属页面、Props / Emits、引用文件数、模板使用次数、引用位置和排序。
- 保留补充内容：按组件路径保留明确的用途说明、备注、自定义列及已说明原因的公共化判断；表格外说明和设计记录原样保留。空白、`待补充` 或“用途需结合源码确认”的用途可由扫描结果补齐。
- 公共化候选：脚本刷新简单的“否/建议评估”标记，并输出引用文件数达到 2 的候选；已经写明原因的判断保留，但引用变化后 Agent 必须重新核查。
- 用途涉及语义，脚本不会用名称推断覆盖已补充说明；Agent 必须结合源码处理用途变化候选。
- 迁移或改名后路径发生变化，Agent 必须将原有用途、备注等有效信息同步到新行，再重扫确认；脚本不凭同名猜测迁移关系。
- 表格结构异常或项目缺少 `src/` 时，脚本报错并保留原文件；Agent 应修复原因后重跑，不得将报错当成维护完成。

仅在用户要求只检查时使用 `--check`，该模式只报告、不创建或修改索引，不能替代开发后的同步。`--force` 是显式全量重建，会丢弃已有补充说明和设计记录，常规维护不得使用。
...
脚本默认输出差异并合并写回正式索引。Agent 必须检查写回结果，并完成需要源码判断的事项：

- 新增组件：补入正式索引。
- 删除组件：从组件表格移除，历史设计记录保留。
...
不能因为正式索引已存在就跳过同步，也不能把“输出了差异”“建议稍后更新”当作完成。验收时确认两张表均按规定排序、引用位置与数量已同步、评估结论和本次设计记录已补齐。只有扫描结果确实无变化，或遇到无法解决的错误并明确说明阻塞原因时，才可以没有文件改动。
...
| 结果 | 处理 |
| --- | --- |
| 明确适合公共化 | 满足下述迁移边界时执行迁移到 `src/components`，同步更新所有调用方引用和组件图谱；不得只标记建议后遗漏已决定的迁移 |
...
执行公共化时必须完成闭环：迁移组件及必要依赖 → 修改全部调用方引用（含测试、注册和导出）→ 检查旧路径残留并运行相关检查 → 将用途与有效备注迁移到新行 → 再次运行扫描脚本。图谱中应移除旧局部组件行、更新公共组件行及引用位置，并重排。扫描器只维护索引和提示候选，语义评估与代码迁移由 Agent 执行，不得仅凭次数自动搬文件。
...
| 只报告差异、不写回索引 | 开发后或提交前运行默认同步命令，核对写回结果与排序，再完成用途、公共化判断和设计记录 |
| 老项目没有索引就跳过复用检查 | 开发前先扫描生成或同步图谱，开发后再次扫描并写回变化 |
