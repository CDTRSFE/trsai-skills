...
10. 开发完成后，更新 `docs/component-map.md`，记录新增、改造、复用或建议公共化的组件。
...
```text
docs/component-map.md（仅当原文件不存在时创建，已存在则不覆盖；如需强制重建可用 `--force`）
```
...
不要覆盖已有正式索引；如果 `docs/component-map.md` 已存在，脚本默认不改文件，正式索引由 Agent 在开发后按实际变更维护。

生成或维护索引表格时，`公共组件` 和 `页面局部组件` 两个表格都必须按 `引用次数` 倒序排列；引用次数相同时按组件路径升序排列，避免高复用组件被低引用组件淹没。
...
组件表格按 `引用次数` 倒序排列；引用次数相同时按路径升序排列。
...
| 组件 | 路径 | 用途 | Props | Emits | 引用情况 | 备注 |
| --- | --- | --- | --- | --- | --- | --- |

## 页面局部组件

| 组件 | 路径 | 所属页面 | 用途 | 引用情况 | 是否建议公共化 |
| --- | --- | --- | --- | --- | --- |
...
1. 读取或生成 `docs/component-map.md` 时，发现某个页面局部组件被两个及以上页面引用。
2. 页面局部组件总引用次数达到 3 次及以上，且不是同一页面内部的简单重复渲染。
3. 当前开发需求需要复用、修改、复制或参考某个页面局部组件。
4. 用户要求做老项目组件梳理、公共组件治理、组件图谱维护、重复组件分析或“看看有没有组件该抽公共”。
5. Agent 准备新建一个与现有页面局部组件相似度 80% 以上的组件。

触发评估后，不得只因为“引用次数多”就直接迁移，必须先判断：
...
| 只新增组件不更新索引 | 开发后必须更新 `docs/component-map.md` |
| 老项目没有索引就跳过复用检查 | 先运行扫描脚本生成初始图谱 |
