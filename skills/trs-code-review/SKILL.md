...
这是收尾审查技能。常驻基础规范放在 `AGENTS.md`；具体实现规范按需调用对应领域技能；本技能负责最终把改动按 TRS 团队标准过一遍。
...
3. 再查团队规范：Vue、TypeScript、样式、自动导入、API、表单、Pinia。
4. 最后查交付质量：测试/构建验证、浏览器运行时验证、提交粒度、MR 描述、风险说明。
...
- diff 是否只包含本次任务相关改动。
- 是否有必要的测试、构建或手动验证说明。
- 涉及页面、组件、样式、交互、接口联调或性能问题的改动，是否使用 `chrome-devtools-mcp` 验证真实浏览器效果，并说明页面路径、核心操作、Console/Network 结果。
...
- 标准流程中，本技能位于 `brainstorming -> writing-plans -> 执行 -> review` 的最后一步，负责检查实际 diff 是否符合 TRS 前端规范。
...
- 完成开发后，先按 `superpowers:verification-before-completion` 做验证，再用本技能做 TRS 规范审查。
