...
- 用户明确要求提交前、GitLab MR 前、合并前、发版前的代码检查。
- 完成开发后需要验收是否符合团队规范。
- 需要把多个领域规范合并成一份审查结果。

用户只说“提交 MR”“创建 MR”或“推送并提交 MR”时，使用 `gitlab-merge-request`，不触发本技能及开发完成验收流程。该流程只处理已有 commit，不自动追加审查、测试、build 或浏览器验证。用户另外明确要求审查时，再使用本技能。
...
- 列表项、卡片、标题行、说明文本、标签组等是否有明确的超长文本处理：flex/grid 文本容器应具备 `min-w-0`，单行内容应截断，多行内容应明确换行或限制行数。
- 同一行“长文本 + 时间/按钮/状态标签”结构中，长文本区域是否 `flex-1 min-w-0`，右侧固定信息是否 `shrink-0`，避免真实数据把容器撑出。
...
- 接口联调完成后，是否清理不再使用的 mock 数据、硬编码展示值、临时接口 fallback、临时 `setTimeout` 和调试日志。
