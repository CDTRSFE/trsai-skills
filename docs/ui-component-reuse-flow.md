...
开发前、开发完成后或提交前，Agent 自动运行：

```bash
node skills/ui-component-reuse/scripts/scan-vue-components.mjs /path/to/project
```

脚本会自动创建 `docs/`；索引不存在时初始化，存在时默认合并更新组件增删、Props / Emits、引用数量与位置，并按引用文件数降序、模板使用次数降序、路径升序重新排列两张表。只调整顺序也会写回；没有变化时不重复写入。

用途说明、备注、自定义列和已说明原因的公共化判断按路径保留，设计记录及表格外说明原样保留。Agent 仍需核查用途变化候选与公共化结论，完成本次设计记录；迁移组件后还需更新调用方引用，将有效说明迁移到新路径，再次扫描。

`--check` 只读检查，不能替代开发后的默认同步。`--force` 会全量重建并丢弃补充内容，常规维护不使用。
